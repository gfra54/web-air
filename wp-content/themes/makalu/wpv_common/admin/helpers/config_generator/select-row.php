...
global $post;
?>
<div class="wpv-config-row <?php echo $class?> select-row top-desc clearfix">
	...
	<div class="rcontent">
		<?php foreach($selects as $id=>$s): ?>
			<?php
				if (isset($s['target'])) {
					if (isset($s['options']))
						$s['options'] = $s['options'] + Config_Generator::get_select_target_config($s['target']);
					else
						$s['options'] = Config_Generator::get_select_target_config($s['target']);
				}
