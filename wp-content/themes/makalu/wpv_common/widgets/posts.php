...
class wpv_posts extends WP_Widget {
	...
	public function wpv_posts() {
		$widget_options = array(
			'classname' => 'wpv_posts',
			'description' => __("Displays a list of posts/comments/tweets", 'wpv')
		);
		$this->WP_Widget('wpv_posts', __('Vamtam - Multi widget', 'wpv') , $widget_options);
	...
	private function get_section_title($orderby) {
		if($orderby == 'comment_count')
			return apply_filters('wpv_multiwidget_tab_title', __('Popular', 'wpv'), $orderby);
		if($orderby == 'date')
			return apply_filters('wpv_multiwidget_tab_title', __('Newest', 'wpv'), $orderby);
		if($orderby == 'comments')
			return apply_filters('wpv_multiwidget_tab_title', __('Comments', 'wpv'), $orderby);
		if($orderby == 'tags')
			return apply_filters('wpv_multiwidget_tab_title', __('Tags', 'wpv'), $orderby);
	}
