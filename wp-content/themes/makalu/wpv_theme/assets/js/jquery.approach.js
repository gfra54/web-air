...
(function($) {
  ...
  $.fn.approach = function(styles, distance, callback) {
    ...
    this.each(function(i, obj) {
      ...
      $.each(styles, function(style, val) {
        ...
        if (from && to) {
          ...
          if (to.relative) to.number = ((to.relative == "-=" ? -1 : 1) * to.number) + from.number;
    ...
    $(document).bind("mousemove", function(e) {
      ...
      if (thisRun - lastRun < settings.interval) return;
      ...
      $.each(elements, function() {
        var self = this,
          ...
          distance = parseInt(Math.sqrt(Math.pow(e.pageX - center.x, 2) + Math.pow(e.pageY - center.y, 2)), 10),
        ...
        $.each($(self).data("jquery-approach"), function() {
          ...
          if ($.isArray(style.to)) {
            color = (distance > settings.distance) ? style.from : $.map(style.from, function(v, k) {
              return parseInt((distanceRatio * (style.to[k] - style.from[k])) + style.from[k], 10);
            });
