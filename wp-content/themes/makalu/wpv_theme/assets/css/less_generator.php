...
class WpvLessGenerator {
	...
	public function __construct() {
		...
		foreach($vars_raw as $var) {
			...
			if(preg_match('/^[-\w\d]+$/i', $var->name)) {
				if(!empty($var->value) && ($value = $this->prepare($var->name, $var->value))) {
					$vars[$var->name] = $value;
				} else {
					$vars[$var->name] = null;
				}
		...
		if(class_exists('lessc') === false)
			require_once(WPV_DIR . 'vendor/leafo/lessphp/lessc.inc.php');
		$l = new lessc();
		$l->importDir = '.';
		...
		foreach ($vars as $name => $value) {
			if(empty($value)) 
				$vars[$name] = isset($wpv_defaults[$name]) ? $this->prepare($name, $wpv_defaults[$name]) : null;
			
			if(!is_null($vars[$name]))
				$out .= empty($value) ? "@$name:false;\n" : "@$name:$value;\n";
		}
	...
	private function prepare($name, $value, $returnOriginal = false) {
		$good = true;
		...
		if(preg_match('/(^share|^has-|^show|-last$)/i', $name)) {
			...
			if (in_array($name, $this->percentages)) {
				$value .= '%';
			}
